...
def _make_ttt_test_model(gamma_treated: float = -0.5) -> FittedPEPHModel:
    ...
    alpha = np.log(np.array([0.01, 0.02], dtype=float))  # baseline hazards
    beta_x1 = 0.3
    beta_sexM = 0.2
...
def test_predict_cumhaz_treated_td_matches_standard_when_never_treated() -> None:
    ...
    wide = pd.DataFrame(
        {
            ...
            "x1": [0.0, 1.0],
            "sex": ["F", "M"],
            "treatment_time": [np.nan, np.nan],  # never treated
        }
    )
    ...
    # Standard prediction should match when nobody is ever treated
    ...
    wide = pd.DataFrame(
        {
            ...
            "x1": [0.0, 1.0],
            "sex": ["F", "M"],
            "treatment_time": [120.0, 999.0],  # after all horizons
        }
    )
    times = [15.0, 45.0, 80.0]
...
def test_predict_cumhaz_treated_td_reduces_hazard_after_treatment_when_gamma_negative() -> None:
    ...
    wide = pd.DataFrame(
        {
            ...
            "x1": [0.5, 0.5],
            "sex": ["F", "F"],
            "treatment_time": [20.0, np.nan],  # subject 1 treated early, subject 2 never treated
        }
    )
    ...
    # Before treatment, hazards should match
    assert np.isclose(ch_td[0, 0], ch_td[1, 0])

    # After treatment, treated subject should have lower cumulative hazard
