...
def _simulate_spatial_treatment_aft_data(
    *,
...
) -> tuple[pd.DataFrame, dict[str, float], list[str], np.ndarray]:
    """
    ...
    -------
    df
        Simulated wide dataframe.
    zip_to_u
        Mapping of ZIP to true spatial effect.
    graph_zips
        ZIP order used in the graph / u vector.
    u_true
        True spatial field in graph order.
    """
    ...
    return df, zip_to_u, graph_zips, np.asarray(u_true, dtype=float)


@pytest.mark.slow
def test_fit_treatment_lognormal_aft_map_leroux_recovery_no_censoring() -> None:
    beta_true = {
    ...
    df, zip_to_u, graph_zips, u_true = _simulate_spatial_treatment_aft_data(
        n=4000,
    ...
    assert abs(beta_hat["Intercept"] - treatment_intercept_true) < 0.20
    assert abs(beta_hat["age_per10_centered"] - beta_true["age_per10_centered"]) < 0.08
    assert abs(beta_hat["cci"] - beta_true["cci"]) < 0.08
    assert abs(beta_hat["ses"] - beta_true["ses"]) < 0.08
    assert abs(beta_hat["sexM"] - beta_true["sexM"]) < 0.08
    assert abs(beta_hat["stageII"] - beta_true["stageII"]) < 0.10
    assert abs(beta_hat["stageIII"] - beta_true["stageIII"]) < 0.12

    # Scale recovery
    assert abs(fitted.sigma - sigma_true) < 0.10

    # Spatial parameter sanity
    assert 0.0 < fitted.spatial.rho < 1.0
    assert fitted.spatial.tau > 0.0

    # Spatial field recovery (up to overall mean shift)
    u_hat = np.asarray(fitted.spatial.u, dtype=float)
    assert len(u_hat) == len(graph_zips)

    # align by graph ZIP order
    fitted_zips = [str(z) for z in fitted.spatial.zips]
    idx = {z: i for i, z in enumerate(fitted_zips)}
    u_hat_aligned = np.array([u_hat[idx[z]] for z in graph_zips], dtype=float)

    corr = np.corrcoef(u_true, u_hat_aligned)[0, 1]
    assert np.isfinite(corr)
    assert corr > 0.50


@pytest.mark.slow
def test_fit_treatment_lognormal_aft_map_leroux_recovery_with_moderate_censoring() -> None:
    beta_true = {
    ...
    # Directional checks under censoring
    assert beta_hat["Intercept"] > 0.0
    ...
    assert abs(fitted.sigma - sigma_true) < 0.15
